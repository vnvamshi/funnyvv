// ═══════════════════════════════════════════════════════════════════════════════
// WhoAreYouModal v7.0 - Voice-First Role Selection with Mic Handoff
// ═══════════════════════════════════════════════════════════════════════════════
...
// Notify main page that modal is taking over mic
const notifyMicTakeover = (active: boolean) => {
  window.dispatchEvent(new CustomEvent('modalMicControl', { detail: { active } }));
};
...
const WhoAreYouModal: React.FC<WhoAreYouModalProps> = ({ isOpen, onClose, onRoleSelected }) => {
  ...
  const handleCommand = useCallback((cmd: string) => {
    ...
    if (c.includes('customer') || c.includes('browse') || c.includes('shop')) {
      ...
      selectRole('builder');
    } else if (c.includes('vendor') || c.includes('sell') || c.includes('supplier') || c.includes('i am a vendor') || c.includes('i\'m a vendor') || c.includes('i am vendor')) {
      selectRole('vendor');
    }
  }, []);

  const voice = useVoice({ owner: 'modal', onCommand: handleCommand, autoStart: isOpen });
  ...
  const handleClose = () => {
    voice.stopListening();
    notifyMicTakeover(false);
    setSelectedRole(null);
    onClose();
  };

  // Notify main page when modal opens/closes
  useEffect(() => {
    if (isOpen) {
      notifyMicTakeover(true);
      setTimeout(() => {
        voice.speak("Who are you? Are you a Customer, Home Buyer, Investor, Real Estate Agent, Builder, or Vendor? Just say your role or click to select.");
      }, 500);
    } else {
      notifyMicTakeover(false);
      setSelectedRole(null);
  ...
  if (selectedRole === 'vendor') {
    return (
      <VendorFlow
        isOpen={true}
        onClose={handleClose}
        onComplete={() => { onRoleSelected?.('vendor'); handleClose(); }}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 10000, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px' }}>
      <div style={{ background: `linear-gradient(135deg, ${THEME.teal}, #001a15)`, borderRadius: '20px', width: '100%', maxWidth: '600px', overflow: 'hidden', border: `2px solid ${THEME.gold}40` }}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: `1px solid ${THEME.gold}30`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={handleClose} style={{ background: 'none', border: 'none', color: '#fff', fontSize: '1.5em', cursor: 'pointer', opacity: 0.7 }}>✕</button>
        </div>
