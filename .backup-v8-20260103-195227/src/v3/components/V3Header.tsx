...
const V3Header = () => {
	...
	const countries = React.useMemo(
		() => (t('header.modal.countries', { returnObjects: true }) as Array<{ country: string; language: string; flag: string }>) || [],
		[t]
	);
	const currencies = React.useMemo(
		() => (t('header.modal.currencies', { returnObjects: true }) as Array<{ name: string; code: string; symbol: string }>) || [],
		[t]
	);
	...
	const [authModalView, setAuthModalView] = React.useState<'signup' | 'login' | 'agentSignUp' | null>(null);
	...
	// Green gradient text style (replacement for gold text)
	...
	return (
		<header className="w-full fixed top-0 left-0 z-50 bg-white h-16">
			<div className="w-full px-6 md:px-8 h-16 flex items-center justify-between border-b border-gray-100">
				{/* Logo */}
				<Link to="/" className="cursor-pointer">
					<img src={logo} alt={t('header.logoAlt')} className="h-13 md:h-12 w-auto" />
				</Link>

				{/* Right - Nav + Controls */}
				<div className="hidden md:flex items-center gap-6">
					{/* Nav - Desktop (solid green text) */}
					<nav className="flex items-center gap-16">
						<a href="#about" className="transition" style={{ color: 'rgba(0, 66, 54, 1)' }}>{t('header.navigation.aboutUs')}</a>
						<a href="#how-it-works" className="transition" style={{ color: 'rgba(0, 66, 54, 1)' }}>{t('header.navigation.howItWorks')}</a>
						<a href="#partners" className="transition" style={{ color: 'rgba(0, 66, 54, 1)' }}>{t('header.navigation.ourPartners')}</a>
						<a href="#lend" className="transition" style={{ color: 'rgba(0, 66, 54, 1)' }}>{t('header.navigation.lendWithUs')}</a>
					</nav>
					...
					<div className="flex items-center gap-3">
						...
						>
							<div className={`rounded-xl px-3 py-0.5 w-full h-9 flex flex-col justify-center`}>
								<div className="flex items-center w-full gap-2">
									<div className="flex gap-2 flex-1 items-center overflow-hidden">
										...
										<div className="flex flex-col min-w-0 flex-1 justify-center leading-tight">
											<span className={`font-medium truncate text-left whitespace-nowrap`} style={{ color: '#004236' }}>
												{selected.country}
											</span>
											<span className={`text-[10px] mt-0 text-left whitespace-nowrap truncate`} style={{ color: 'rgba(0,66,54,0.7)' }}>
									...
									{countries.map((o) => {
										...
										return (
											<button
												...
												style={
													isActive
														? { background: 'linear-gradient(90deg, #905E26 0%, #F5EC9B 25.82%, #905E26 100%)', boxShadow: '3px 3px 5px 0px #F5EC9B33 inset, -3px -3px 5px 0px #00000040 inset, 10px 10px 10px 0px #0C656E1A' }
													: { background: 'linear-gradient(111.83deg, rgba(0, 66, 54, 0.5) 11.73%, rgba(0, 126, 103, 0.5) 96.61%)' }
												}
											>
												<div className="flex items-center gap-3">
													...
													<div className="flex flex-col items-start text-left">
														{isActive ? (
															<>
																<span className={`text-[rgba(0,66,54,1)] text-sm font-bold`}>{o.country}</span>
																<span className={`text-[rgba(0,66,54,1)] text-[10px]`}>{o.language}</span>
															</>
														) : (
															<>
																<span className={`text-gold-gradient text-sm font-bold`}>{o.country}</span>
																<span className={`text-gold-gradient text-[10px]`}>{o.language}</span>
															</>
														)}
													</div>
												</div>
			...
			{authModalView === 'signup' && (
				<V3SignupModal
					...
					onClose={() => setAuthModalView(null)}
					onSignInClick={() => setAuthModalView('login')}
					onSwitchToAgentSignUp={() => setAuthModalView('agentSignUp')}
				/>
			)}
            {authModalView === 'login' && (
