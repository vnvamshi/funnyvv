...
export function useSignupForm() {
  ...
  const form = useForm<SignupFormValues>({
    resolver: yupResolver(getSignupValidationSchema(t)) as any,
  });
  ...
  const onSubmit = async (
    ...
    try {
      ...
      const response = await api.post('/common/buyer/signup/', { 
        name: data.name,
        email: data.email,
