...
import AboutAgentModal from './AboutAgentModal';
import AboutAgentMobileModal from './AboutAgentMobileModal';
import useIsMobile from '../../../hooks/useIsMobile';

interface AboutAgentTabProps {
  user: any;
  onProfileUpdated?: () => void;
}

const AboutAgentTab: React.FC<AboutAgentTabProps> = ({ user, onProfileUpdated }) => {
  const [showAboutModal, setShowAboutModal] = useState(false);
  const isMobile = useIsMobile();

  // Handler to close modal and refresh user data
  ...
  return (
    <div className="w-full mx-auto pl-10 pr-10">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="font-semibold text-base text-[#00473E]">Get to know about agent</div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-[#D0F2EB] rounded-lg p-4 flex flex-col min-h-[90px]">
          ...
          <div className="flex gap-2 flex-wrap">
            {user?.specialties && user.specialties.length > 0 ? (
              user.specialties.map((spec: string) => (
                <span key={spec} className="bg-[#85C6BA] text-primary-color font-bold rounded-md px-3 py-2 text-medium">{spec}</span>
              ))
            ) : (
        ...
        <div className="bg-[#D0F2EB] rounded-lg p-4 flex flex-col min-h-[90px]">
          <div className="flex items-center mb-2">
            <div className="text-sm">Agent speaks</div>
          </div>
          <div className="font-semibold text-base">
            {user?.languages && user.languages.length > 0 ? user.languages.join(', ') : <span className="text-gray-400">No languages listed</span>}
          </div>
        </div>
        {/* Experience as agent */}
        <div className="bg-[#D0F2EB] rounded-lg p-4 flex flex-col min-h-[90px]">
          <div className="text-sm">Experience as agent</div>
          <div className="font-semibold text-base">
            {user?.business_since ? (
              <>Since {user.business_since}, {new Date().getFullYear() - user.business_since} yrs experience as a professional agent</>
            ) : (
        ...
        <div className="bg-[#D0F2EB] rounded-lg p-4 flex flex-col min-h-[90px]">
          ...
          <div className="font-semibold text-base">
            {user?.user_service_areas && user.user_service_areas.length > 0 ? (
              user.user_service_areas.map((area: any) => area.name).join(', ')
            ) : (
      ...
      {isMobile ? (
        <AboutAgentMobileModal open={showAboutModal} onCancel={handleModalClose} user={user} onUpdated={onProfileUpdated} />
      ) : (
        <AboutAgentModal open={showAboutModal} onCancel={handleModalClose} user={user} />
      )}
    </div>
  );
};

export default AboutAgentTab; 